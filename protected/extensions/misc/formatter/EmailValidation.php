<?php
class EmailValidation extends CWidget{
	#
	...
	public static function is_valid($email){
		if(self::is_valid_email_address($email) && 
		   self::is_valid_email_address2($email) &&
		   self::is_valid_email_address3($email)){
			return true;
		}
		else{
			return false;
	...
	public static function is_valid_email_address2($email){
	...
	public static function is_valid_email_address3($email){
		$pattern  = '/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-][A-Za-z0-9.-]+\.[A-Za-z]{2,4}\b/';
		if(preg_match($pattern,$email) && filter_var($email, FILTER_VALIDATE_EMAIL)) 
			return true;
		return false;
	}
